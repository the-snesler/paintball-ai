...
export function PromptVariationsToggle() {
  ...
  const tooltipContent = (
    <>
      <p className="text-text-primary mb-1.5 font-medium">Prompt Variations</p>
      <p>Generate unique prompts for each image by adding variable sections to your prompt:</p>
      <code className="bg-surface-raised text-text-tertiary my-1.5 block rounded px-2 py-1">
        {"{{sunset: vary time of day}}"}
      </code>
      <p>
        The text model will replace each{" "}
        <code className="bg-surface-raised text-text-tertiary rounded px-1">{"{{}}"}</code> section
        with a unique variation per image. It will use the example text before the colon as a base
        for generating variations, and the text after the colon as instructions for how to vary it.
      </p>
      <p>
