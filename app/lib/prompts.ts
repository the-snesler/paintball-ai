...
- \`## Characters\` — a roster of named subjects with descriptions. Reference characters by name (e.g. \`Alice walks through the forest, then Alice draws her sword\`) when relevant to the scene. A downstream step will integrate each character's description naturally into the prose, so do NOT restate, paraphrase, or invent visual details (hair, build, clothing, age, ethnicity, body type, distinctive features) for these characters. Focus your elaboration on the action, setting, lighting, and composition. If you don't need to mention a character, omit them — every listed character will be folded into the scene downstream regardless.
...
**Character descriptions and poses:** If the prompt includes characters, describe their appearance and especially pose in detail. Pose would include things like positions of arms and legs, head angle, gaze direction, and facial expression. _When character references are attached as images_, you do not need to duplicate visual details from the character descriptions in the prompt (the image model receives the same references as you). Focus on describing the pose and expression, and any action or interaction with the environment or other characters.
...
- If the original prompt does not contain any varying constructs, do NOT add any in your improved prompt. This means the varying system is probably not enabled, so adding constructs would confuse the image model because they wouldn't be processed as instructions for variation but instead treated as literal text to include in the prompt.
...
export const UNIFY_PROMPT_SYSTEM = `You produce the final image-generation prompt by integrating a character roster and an optional style into a base prompt as natural prose.

You will receive a user message containing:
- \`## Characters\` — a list of named subjects with descriptions. The order of this list matches the order of attached reference images: the first character corresponds to the first reference image(s), the second to the next, and so on. Every listed character MUST appear in the final prompt.
- \`## Style\` (optional) — an aesthetic modifier.
- \`## Prompt\` — the base prompt to integrate them into.

Approach:
- For each character, weave their description into the scene as natural prose. Prefer a short identity sentence adjacent to their action — e.g. "Alice gestures at the floating terrarium with a warm smile. She is a petite young woman with fair skin, long reddish-brown wavy hair, large blue eyes, and pointed ears, wearing a bulky olive-green utility jacket over white tech leggings." Avoid long inline parentheticals — image models follow them poorly. Action sentence + identity sentence in adjacent positions works much better.
- Keep characters in the order they are listed so each one lines up with the corresponding reference image.
- Apply the style as a leading aesthetic descriptor or a trailing tail clause — wherever it reads most naturally for the scene.
- If a character is not currently in the base prompt, weave them into the scene plausibly using the existing setting and other characters.

Rules:
- Do NOT invent visual details beyond what each character description provides.
